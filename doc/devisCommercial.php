...
<page backtop="0mm" backleft="0mm" backright="0mm" backbottom="20mm" style="/*font-family: freeserif*/">
	...
	<?php foreach ($list as $value) { 
		...
		<table class="addPadding">
			<tr>
				<td style="width: 100%; text-align: left; font-weight: bold;">A l'attention de : <?php $t = explode(' #$# ', html_entity_decode($value['contact']));
																										echo $t[0] ?></td>
				<!-- " - " . $t[1] -->
			</tr>
		</table>
		...
		<?php $i = 0;
			...
			<table class="addPadding" cellpadding="0" cellspacing="0">
				<tr style="background-color: #26c0c9; color: #ffffff;">
					<td style="padding: 5px; padding-top: 1px; padding-bottom: 1px; width: 85%; font-weight: bold;"><?php echo html_entity_decode(decodeCaractere($valueRubrique['rubrique'])) ?></td>
					<td style="padding: 5px; padding-top: 1px; padding-bottom: 1px; width: 15%; text-align: right;font-weight: bold;"><?php echo number_format($valueRubrique['total'], 0, ',', ' ') ?> F</td>
				</tr>
			</table>
			<?php
					...
					// SELECT DISTINCT(d.idFamille), f.famille ,t.typeService FROM detailstypeservice d, famille f ,typeService t
					// WHERE d.idDevis = \"$idDevis\" AND d.idRubrique =\"$idRubrique\" AND d.idFamille = f.idFamille AND t.idtypeService = d.idTypeservice AND t.idTypeservice = \"$idTypeservice\"
						...
						<!-- <tr>
							...
							<td style="padding: 5px; width: 15%; text-align: right;">
								<?php
												...
												echo "<strong style='text-align:center;'>" . html_entity_decode(decodeCaractere($valueTypeservice['quantite'])) . "</strong><br>";
												?>
							</td>
				...
				<td style="width: 60%;">
					...
					<?php if (intval($value['taxeMunicipale']) != 0) { ?>
						<table style="width: 100%;" cellpadding="0" cellspacing="0">
							<tr style="background-color: #26c0c9; color: #ffffff;">
								...
								<td style="padding: 5px; width: 30%; text-align: right;">
									<?php echo number_format(($value['montantDevis'] * $value['taxeMunicipale']) / 100, 0, ',', ' ') ?> F
								</td>
							</tr>
					...
					<!-- <table style="width: 100%;" cellpadding="0" cellspacing="0">
						<tr>
							...
							<td style="padding: 5px; width: 30%; text-align: right;">
								<?php echo number_format(($value['montantDevis'] * (0.8)), 0, ',', ' ') ?>
