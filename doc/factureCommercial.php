...
if (isset($_GET['accompte'])) {
	$accompte = $_GET['accompte'];
}
if (isset($_GET['idDevis'])) {
	$idDevis = $_GET['idDevis'];
	$requete = Connexion::Connect()->query("SELECT * FROM vfacture WHERE idDevis = \"$idDevis\" ");
...
<page backtop="0mm" backleft="0mm" backright="0mm" backbottom="20mm" style="/*font-family: freeserif*/">
	...
	<?php foreach ($list as $value) { ?>
		<table style="width: 76%;" class="addPadding">
			...
			<tr>
				...
				<td style=" width: 30%; text-align: right;">
					Dakar, le <?php echo DateComplete($value['dateFacture']) ?>
				</td>
			</tr>
			<tr>
		...
		<table style="width: 84.5%;" class="addPadding" cellpadding="0" cellspacing="0">
			<tr>
				<td style="background-color: #24bfc8; width: 50%; color: #fff;">
					FACTURE N&deg; : <?php echo $value['numeroFacture'] ?>
				</td>
			</tr>
		</table>
		<table style="width: 84.5%;" class="addPadding" cellpadding="0" cellspacing="0">
			<tr>
				<td style="width: 50%;">
					VOS REF : BC N&deg; <?php echo $value['numBc'] ?>
				</td>
			</tr>
		</table>
		<table style="width: 84.5%;" class="addPadding" cellpadding="0" cellspacing="0">
			<tr>
				<td style="width: 50%;">
					NOS REF : Devis n&deg; <?php echo $value['numeroDevis'] ?>
				</td>
			</tr>
		</table>
		<br>
		<table class="addPadding">
			<tr>
				<td style="padding-top: 1px; padding-bottom: 1px; padding-right: 30px; min-width: 40%; text-align: left; font-weight: bold; background-color: #26c0c9; color: #ffffff;">A l'attention de : <?php if (empty($value['destinataire'])) {
																																																					$t = explode(' #$# ', $value['contact']);
																																																					echo $t[0];
																																																				} else {
																																																					echo $value['destinataire'];
																																																				} ?>
				</td>
			</tr>
		</table>
				...
				<?php foreach ($listTypeservice as $valueTypeservice) { ?>
					<table class="addPadding" cellpadding="0" cellspacing="0">
						<tr>
							...
							<td style="padding: 5px; width: 60%;font-weight: bold"><?php echo html_entity_decode(decodeCaractere($valueFamille['famille'])) ?></td>
					...
					<table style="width: 100%;" cellpadding="0" cellspacing="0">
						...
						<?php if (intval($value['hasRemise']) != 0) { ?>
							<tr>
								...
								<td style="padding: 5px; width: 30%; text-align: right;">
									<?php
											if ($value['typeRemise'] == 1) {
												$remise = ($value['montantDevis'] * $value['valeurRemise'])/100;
											} else if ($value['typeRemise'] == 2) {
												$remise = $value['valeurRemise'];
											}
											echo number_format($remise, 0, ',', ' ') ?> F
						...
						<tr>
							...
							<td style="padding: 5px; width: 30%; text-align: right;">
								<?php echo number_format(($value['montantDevis'] * 18) / 100, 0, ',', ' ') ?> F
							</td>
						</tr>
						<tr style="font-weight: bold;">
							<td style="padding: 5px; width: 70%;">
							...
							<td style="padding: 5px; width: 30%; text-align: right;">
								<?php echo number_format(($value['montantDevis'] + (($value['montantDevis'] * 18) / 100) + (($value['montantDevis'] * $value['commissionProduction']) / 100)), 0, ',', ' ') ?> F
							</td>
						</tr>
						<?php $accompte =($value['montantDevis'] * $value['accompte'])/100;
							if (intval($accompte) != 0) { ?>
							<tr>
								<td style="padding: 5px; width: 70%; font-style: italic;">
									ACCOMPTE
								</td>
								<td style="padding: 5px; width: 30%; text-align: right;">
									<?php echo number_format($accompte, 0, ',', ' ') ?> F
								</td>
							</tr>
						<?php } ?>
						<tr style="font-weight: bold; background-color: #481511; color: #fff;">
							...
							<td style="padding: 5px; width: 30%; text-align: right;">
								<?php echo number_format(($value['montantDevis'] + (($value['montantDevis'] * 18) / 100) + (($value['montantDevis'] * $value['commissionProduction']) / 100) - (intval($accompte))), 0, ',', ' ') ?> F
							</td>
						</tr>
					</table>
				</td>
		...
		<table class="addPadding" cellpadding="0" cellspacing="0">
			<tr>
				<td style="width: 50%;">
					...
					<?php
						$obj = new nuts($value['montantDevis'] + (($value['montantDevis'] * 18) / 100) + (($value['montantDevis'] * $value['commissionProduction']) / 100) - (intval($accompte)), "EUR");
						$text = $obj->convert("fr-FR");
						$nb = $obj->getFormated(" ", ",");
						echo "<span>" . ucfirst($text) . "</span>";
						?>
				</td>
				<td style="width: 20%;">
				</td>
		...
		<table class="addPadding" cellpadding="0" cellspacing="0">
			<tr>
				<td>
					<span style="font-weight: bold;">Conditions de paiement</span> : <br><br>
