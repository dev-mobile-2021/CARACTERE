<?php
	...
	La fonction func_get_arg() recupère la valeur de l'argument à la position qui lui est donnée en paramètre*/
		...
		public function listClient(){
			...
			$requete = Connexion::Connect()->query('SELECT  nomClient, SUM(montantDevis) as total FROM vdevis GROUP BY nomclient');
			...
			// Liste des clients 
			public function venteJanvier(){
				$list = array();
	
				$requete = Connexion::Connect()->query('SELECT montantDevis as janvier ,nomClient, dateAjout FROM vdevis WHERE MONTH(dateAjout) = 1 GROUP BY nomClient,MONTH(dateAjout) =1;') ;
