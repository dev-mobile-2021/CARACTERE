...
if (isset($_POST["export_excel"])) {
    $requete = Connexion::Connect()->prepare("SELECT * FROM vdevis2 WHERE  idDevis IN (SELECT idDevis FROM devisbc WHERE etat = 1)");
    $requete->execute();
    $devis = $requete->fetchAll();
     ...
     if (count($devis) > 0) {
      foreach ($devis as $value) {
        ...
        header("Content-Disposition: attachment; filename=factureavecBC.txt");
    ...
    //                 header("Content-Disposition: attachment; filename=afactureravecbc.xls");
    //                 echo $output;
    // }
}
