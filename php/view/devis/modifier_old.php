...
<div class="content-wrapper">
  ...
  <section class="content">
    ...
    ?>
    <?php foreach ($list as $value) { $idDevis = $value['idDevis']; ?>
      <section class="content">
                    ...
                    <?php foreach ($listTypeservice as $valueTypeservice) { $idTypeservice = $valueTypeservice['idTypeservice']; ?>
                      ...
                      ");
                            foreach ($listService as $valueService) {
                              ...
                              ?>
                            <p>
                              <!-- <a href="javascript:void(0);" onclick="modifierService(<?php //echo $idRubrique ?>, <?php //echo $idFamille ?>, <?php //echo $idTypeservice ?>, <?php //echo $idService ?>, <?php //echo $idDevis ?>, <?php //echo intval($valueService['prixAchat']) ?>, <?php //echo intval($valueService['prixVente']) ?>, <?php //echo intval($valueService['quantite']) ?>)" style="margin-left:10px"><i style = "color:#000000" class="glyphicon glyphicon-pencil optionModSup" data-toggle="tooltip" title="Modifier"></i> </a> -->
                      ...
                      <div class="col-md-10" style="padding-right: 0px;">
                        ...
                        <div class="col-md-3" style="padding-left: 0px;">
                          <div class="form-group">
                            ...
                            <input type="number" value="0" class="form-control" id="prixAchatService" name="prixAchatService" placeholder="Entrer le prix d'achat" required="required" min="0">
                          </div>
                        </div>  
                        <div class="col-md-3" style="padding-left: 0px;">
                          <div class="form-group">
...
function saveDetails(){
  if($("#monFormService #idService").val() == null /*|| $("#monFormService #prixAchat").val().length === 0 || $("#monFormService #prixVente").val().length === 0 || $("#monFormService #quantite").val().length === 0*/){
    // alert("remplir tout");
    ...
    var idService = $("#monFormService #idService").val();
    ...
    var texteSousService = $("#monFormService #idService :selected").text();
    // alert(texteSousService);

    //Enregistrement du sous service dans un tableau
    var toAdd = tableauSousService[idService] = idService+"+@+"+prixAchat+"+@+"+prixVente+"+@+"+quantite;
...
function updateLine(id){
  ...
  $("#monFormUpdateLine #idServiceMod").val(valeurs[0]).change();
...
function saveDetailsUpdate(){
  ...
  if($("#monFormUpdateLine #idServiceMod").val() == null /*|| $("#prixAchat").val().length === 0 || $("#prixVente").val().length === 0 || $("#quantite").val().length === 0*/){
    swal({ title: "Erreur", text: "Veuillez choisir un descriptif", imageUrl: 'dist/img/icones/error.png', html: true});
    // alert("remplir tout");
  }else{
    var val = $("#sousServices").attr("value");
    var idServiceMod = $("#monFormUpdateLine #idServiceMod").val();
    ...
    var texteSousService = $("#monFormUpdateLine #idServiceMod :selected").text();
    // alert(texteSousService);
    ...
    //Enregistrement du sous service dans un tableau
    var toAdd = tableauSousService[idServiceMod] = idServiceMod+"+@+"+prixAchat+"+@+"+prixVente+"+@+"+quantite;
    var toAdd = JSON.stringify(tableauSousService);
    /*alert(toAdd);*/
    //Affichage du sous service
    var prixAchatTexte = number_format(prixAchat, 0, '', ' ');
    ...
    $("#monFormUpdateLine #sousServicesAjoutes").append('<div onclick="updateLine('+idServiceMod+')" data-toggle="tooltip" title="Cliquez pour modifier" class="row ligneSousService" id="id'+idServiceMod+'" style="margin-left: 0px; margin-right: 0px; margin: 5px 0px 5px 0px;"><div class="col-md-10" style="padding-right: 0px;"><div class="col-md-3" style="padding-left: 0px;">'+texteSousService+'</div><div class="col-md-3" style="padding-left: 0px; text-align:right;">'+prixAchatTexte+' F</div><div class="col-md-3" style="padding-left: 0px; text-align:right;">'+prixVenteTexte+' F</div><div class="col-md-3" style="padding-left: 0px; text-align:right;">'+quantiteTexte+'</div></div><div class="col-md-2" style="padding-right: 0px;"><div class="col-md-12 montantSousService" data-value="'+parseInt(prixVente)*parseInt(quantite)+'" style="padding-left: 0px; text-align:right;">'+totalLigne+' F</div></div></div>');
    $(".ligneSousService").tooltip();
    //Desactivation du sous service dans la liste
    $('#monFormUpdateLine #idServiceMod').prop('selectedIndex',0);
    $('#monFormUpdateLine #idServiceMod').select2();

    //Reitialisation des champs
    
    $("#monFormUpdateLine #idServiceMod").val('');
...
function hotDeleteLine(){//--------------------------------------------------- A refaire avec confirm
  var id = $("#monFormUpdateLine #oldIdSousService").attr("value");
  if($("#monFormUpdateLine #idServiceMod").val() == null){
    swal({ title: "Erreur", text: "Veuillez choisir un descriptif", imageUrl: 'dist/img/icones/error.png', html: true});
    // alert("remplir tout");
  }else{
    //suppression du sous service
...
function sendAll(){
  ...
  if(((totalService == totalSousService) && (totalService != 0 && totalSousService != 0)) || (totalSousService == 0 && totalService != 0) || (totalSousService != 0 && totalService == 0) ){
    ...
    swal({ title: "Erreur", text: "Le prix du service ne correspond pas &agrve; la sommes des prix de ses Descriptifs<br> Prix du service : "+totalService+" <br> Total des Descriptifs : "+totalSousService, imageUrl: 'dist/img/icones/error.png', html: true});
...
function askPrintDevis(){
  ...
  swal({   title: "Devis enregistr&eacute; avec succ&egrave;s",   
